...
REM   MAX_PORT_ATTEMPTS  default 10
...
if not defined TENANT_ID (
    ...
    1>&2 echo        Example: delete_cache.cmd be01bf45-611a-4011-90a8-76227db1d190 CustomerIdAndEmaiMap
    exit /b 1
)
...
if %errorlevel%==0 (
    1>&2 echo [luz-del-cache] %HOST%:%PORT% already reachable - reusing.
    goto :have_port
)

set "LOG=%TEMP%\luz-portforward-%NAMESPACE%-%PORT%.log"
type nul > "%LOG%"
1>&2 echo [luz-del-cache] launching port-forward local:%PORT% -^> svc:api-forwarder:%REMOTE_PORT% ^(ns=%NAMESPACE%^)...
...
if %errorlevel%==0 (
    1>&2 echo [luz-del-cache] port-forward ready on local port %PORT% (after !_w!s)
    goto :have_port
)
...
if %errorlevel%==0 (
    ...
    if !_attempts! LSS %MAX_PORT_ATTEMPTS% (
        1>&2 echo [luz-del-cache]   port busy - trying !PORT!...
        goto :try_port
    )
    1>&2 echo [luz-del-cache] ERROR: tried %MAX_PORT_ATTEMPTS% ports starting at %_START_PORT% - none worked.
    exit /b 1
)

1>&2 echo [luz-del-cache] ERROR: port-forward on %PORT% failed - see %LOG%
type "%LOG%" 1>&2
exit /b 1
...
if not defined TOKEN (
    if not defined ADMIN_TENANT_ID (
        1>&2 echo [luz-del-cache] ERROR: TOKEN not set and ADMIN_TENANT_ID missing - cannot authenticate.
        1>&2 echo                  Pass TOKEN=... or ADMIN_TENANT_ID=...
        exit /b 2
    )
    set "TOKEN_SCRIPT=%~dp0..\luz-skill-get-token\get_token.cmd"
    if not exist "!TOKEN_SCRIPT!" (
        1>&2 echo [luz-del-cache] ERROR: companion script not found at !TOKEN_SCRIPT!
        exit /b 2
    )
    1>&2 echo [luz-del-cache] Acquiring token via luz-skill-get-token...
    for /f "usebackq delims=" %%T in (`call "!TOKEN_SCRIPT!" "%ADMIN_TENANT_ID%"`) do set "TOKEN=%%T"
    if not defined TOKEN (
        1>&2 echo [luz-del-cache] ERROR: token acquisition failed.
        exit /b 2
    )
)

REM ─── DELETE cache entry ──────────────────────────────────────
set "URL=http://%HOST%:%PORT%/luz_cache/api/%TENANT_ID%/%CACHE_KEY%"
1>&2 echo [luz-del-cache] About to evict cache entry:
1>&2 echo   tenant     = %TENANT_ID%
1>&2 echo   cache key  = %CACHE_KEY%
1>&2 echo   namespace  = %NAMESPACE%
1>&2 echo   url        = %URL%

set "BODY_FILE=%TEMP%\luz-del-cache-%RANDOM%.body"
for /f "usebackq delims=" %%C in (`curl -sS -o "%BODY_FILE%" -w "%%{http_code}" --location --request DELETE "%URL%" --header "Authorization: %TOKEN_PREFIX%%TOKEN%"`) do set "HTTP_CODE=%%C"

1>&2 echo [luz-del-cache] HTTP %HTTP_CODE%
...
1>&2 echo [luz-del-cache] response body:
